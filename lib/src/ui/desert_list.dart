...
class DesertList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return new MaterialApp(
        home: DesertListFiltered(),
        theme: ThemeData(primarySwatch: Colors.deepOrange));
  }
}

class DesertListFiltered extends StatefulWidget {
  @override
  State createState() => _StateDesertListFiltered();
}

class _StateDesertListFiltered extends State<DesertListFiltered> {
  final TextEditingController _filter = new TextEditingController();
  Icon _searchIcon = new Icon(Icons.search);
  String _searchText = "";
  Widget _appBarTitle = new Text('Desert Catalogue');
  List<MealsListBean> names = new List();
  List<MealsListBean> filteredNames = new List();
  int countItem = 0;


  _StateDesertListFiltered() {
    _filter.addListener(() {
  ...
  Widget _buildBar(BuildContext context) {
    return new AppBar(
      centerTitle: true,
      title: _appBarTitle,
      leading: new IconButton(key: Key('search'), icon: _searchIcon, onPressed: _searchPressed),
    );
  }
  ...
  void _searchPressed() {
    setState(() {
      if (this._searchIcon.icon == Icons.search) {
        this._searchIcon = new Icon(Icons.close, key: Key('closesearch'),);
        this._appBarTitle = new TextField(key: Key('textsearch'),
          controller: _filter,
        ...
        this._appBarTitle = new Text('Desert Catalogue');
        filteredNames = names;
        _filter.clear();
      }
    });
